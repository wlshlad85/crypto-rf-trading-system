"""Fast optimization suite for Crypto Random Forest Trading System."""
...
class FastCryptoOptimizer:
    """Fast optimization suite for cryptocurrency Random Forest models."""
    ...
    async def run_fast_optimization(self):
        """Run fast optimization suite."""
        print("\n" + "="*80)
        print("🚀 CRYPTOCURRENCY RANDOM FOREST - FAST OPTIMIZATION SUITE")
        print("="*80 + "\n")
        ...
        # Step 2: Fast hyperparameter optimization
        print("\n🔧 Step 2: Running Fast Hyperparameter Optimization...")
        hp_results = await self._fast_optimize_hyperparameters()
        
        # Step 3: Feature optimization
        print("\n🎯 Step 3: Optimizing Feature Selection...")
        feature_results = await self._fast_optimize_features()
        
        # Step 4: Target optimization
        print("\n📈 Step 4: Optimizing Target Variables...")
        target_results = await self._fast_optimize_targets()
        
        # Step 5: Ensemble optimization
        print("\n🤖 Step 5: Building Optimized Ensemble...")
        ensemble_results = await self._fast_optimize_ensemble()
        
        # Step 6: Strategy optimization
        print("\n💼 Step 6: Optimizing Trading Strategy...")
        strategy_results = await self._fast_optimize_strategy()
    ...
    async def _prepare_data(self):
        ...
        # Use 3 main cryptocurrencies for faster optimization
        self.config.data.symbols = ['bitcoin', 'ethereum', 'solana']
        self.config.data.days = 90  # 3 months for faster processing
    ...
    async def _fast_optimize_hyperparameters(self):
        """Fast hyperparameter optimization using Optuna."""
        print("   🔍 Running fast Bayesian optimization...")
        
        def objective(trial):
            # Suggest hyperparameters - reduced search space
            params = {
            ...
            # Train and validate
            try:
                results = model.train(X_clean, y_clean, validation_split=0.25)
                
                # Calculate trading performance
                val_size = int(len(X_clean) * 0.25)
        ...
        study.optimize(objective, n_trials=15)  # Reduced from 30
    ...
    async def _fast_optimize_features(self):
        """Fast feature selection optimization."""
        print("   🎯 Testing different feature counts...")
        
        # Test fewer feature counts for speed
        ...
        for n_features in feature_counts:
            ...
            try:
                ...
                if len(X_clean) > 600:
                    X_clean = X_clean.iloc[-600:]
                    y_clean = y_clean.iloc[-600:]
    ...
    async def _fast_optimize_targets(self):
        """Fast target variable optimization."""
        print("   📈 Testing different target configurations...")
        
        # Test fewer horizons for speed
        horizons = [3, 6, 12]  # Hours
        target_types = ['returns']  # Only regular returns for speed
        ...
        for horizon in horizons:
            for target_type in target_types:
                ...
                try:
                    ...
                    if len(X_clean) > 500:
                        X_clean = X_clean.iloc[-500:]
                        y_clean = y_clean.iloc[-500:]
    ...
    async def _fast_optimize_ensemble(self):
        """Fast ensemble optimization."""
        ...
        # Test fewer ensemble sizes for speed
        ensemble_sizes = [3, 5]
        results = {}
        ...
        for size in ensemble_sizes:
            ...
            try:
                ...
                # Use smaller dataset for speed
                if len(X_clean) > 800:
                    X_clean = X_clean.iloc[-800:]
                    y_clean = y_clean.iloc[-800:]
        ...
        best_ensemble_size = max(results, key=results.get) if results else 3
    ...
    async def _fast_optimize_strategy(self):
        """Fast trading strategy optimization."""
        print("   💼 Optimizing trading strategy parameters...")
        
        # Test fewer thresholds for speed
        thresholds = [(0.7, 0.3), (0.75, 0.25), (0.8, 0.2)]
        results = {}
    ...
    def _generate_optimization_report(self):
        ...
        fig.suptitle('Cryptocurrency Random Forest - Fast Optimization Results', fontsize=16)
        ...
        if 'features' in self.optimization_results:
            ...
            ax2.bar(range(len(counts)), scores, color='green', alpha=0.7)
        ...
        if 'targets' in self.optimization_results:
            ...
            ax3.bar(range(len(targets)), scores, color='orange', alpha=0.7)
        ...
        if 'strategy' in self.optimization_results:
            ...
            ax4.bar(range(len(thresholds)), sharpe_ratios, color='red', alpha=0.7)
        ...
        viz_filename = f"fast_optimization_results_{timestamp}.png"
        plt.savefig(viz_filename, dpi=150, bbox_inches='tight')
        print(f"   ✅ Visualization saved to {viz_filename}")
        
        # Save optimization results
        results_filename = f"fast_optimization_results_{timestamp}.json"
        ...
        for key, value in self.optimization_results.items():
            if key == 'hyperparameters' and 'study' in value:
                ...
                for k, v in value['results'].items():
                    strategy_results[f"{k[0]}_{k[1]}"] = v
                serializable_results[key] = {
                    'results': strategy_results,
                    'best_thresholds': f"{value['best_thresholds'][0]}_{value['best_thresholds'][1]}"
                }
            else:
        ...
        # Print summary
        print("\n" + "="*80)
        print("🏆 FAST OPTIMIZATION COMPLETE - SUMMARY")
        print("="*80)
        ...
        if 'strategy' in self.optimization_results:
            ...
            strategy_perf = self.optimization_results['strategy']['results'].get(best_thresholds, {})
            print(f"\n💼 Optimal Strategy Thresholds: {best_thresholds}")
            if strategy_perf:
                print(f"   • Return: {strategy_perf.get('total_return', 0):.2%}")
                print(f"   • Sharpe: {strategy_perf.get('sharpe_ratio', 0):.2f}")
                print(f"   • Win Rate: {strategy_perf.get('win_rate', 0):.2%}")
...
async def main():
    """Run the fast optimization suite."""
    optimizer = FastCryptoOptimizer()
    results = await optimizer.run_fast_optimization()
    
    print("\n✨ Fast optimization suite completed successfully!")
    return results
